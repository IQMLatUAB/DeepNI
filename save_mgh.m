...
%
mgz_flag = false;
if strcmp(lower(fname(end-2:end)), 'mgz')
    output_mgz_filename = fname;
    fname = [fname(1:end-1) 'z'];
    mgz_flag = true;
end
r = 1;
...
if mgz_flag
    gzip(fname);
    movefile([fname '.gz'], output_mgz_filename);
end
...
% 	ind = findstr(fname, '.');
% 	new_fname = fullfile(tempdir, sprintf('%d.mgh', gzipped));
