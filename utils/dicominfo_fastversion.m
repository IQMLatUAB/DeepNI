function metadata = dicominfo_fastversion(filename, do_fields_table, now_dict, varargin)
...
try
    ...
    if (~isdicom(fileDetails.name))
        metadata = []; return; %error(message('images:dicominfo:notDICOM'))
    end
    ...
    vec1 = sum(do_fields_table.*repmat([65536 1], [size(do_fields_table,1) 1]), 2);
    
    temptemp1 = double(vertcat(attrs(1:end).Group))*65536+double(vertcat(attrs(1:end).Element));
    
    idx_found = find(ismember(temptemp1,vec1));
%     
    % Process the raw attributes.
    [metadata,attrNames] = processMetadata(attrs(idx_found), true, now_dict, args.UseDictionaryVR, specificCharacterSet);
